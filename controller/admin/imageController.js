...
const softDeleteImage = async (req, res) => {
  try {
    ...
    let updatedImage = await deleteDependentService.softDeleteImage(query, updateBody);
    if (!updatedImage){
      return res.recordNotFound();
    }
    return  res.success({ data :updatedImage });
...
const deleteImage = async (req, res) => {
  try {
    ...
    if (dataToDeleted && dataToDeleted.isWarning) {
      let countedImage = await deleteDependentService.countImage(query);
      if (!countedImage){
        return res.recordNotFound();
      }
      return res.success({ data :countedImage });
    }
    let deletedImage = await deleteDependentService.deleteUser(query);
    if (!deletedImage){
      return res.recordNotFound(); 
    }
...
const deleteManyImage = async (req, res) => {
  try {
    ...
    let deletedImage = await deleteDependentService.deleteImage(query);
    if (!deletedImage) {
      return res.recordNotFound();
    }
    return res.success({ data: deletedImage });          
